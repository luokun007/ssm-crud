...
<body>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="email" name="email" class="form-control"
									id="empEmail_update_input" placeholder="Email"> <span
				...
				<div class="modal-body">
					<form class="form-horizontal">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="email" name="email" class="form-control"
									id="empEmail_add_input" placeholder="Email"> <span
									class="help-block"></span>
							</div>
						</div>
						<div class="form-group">
	...
	<script type="text/javascript">
		//1.页面加载后，直接发送一个ajax情求，要到分页数据
		var maxPage, currrentNum;
		$(function() {
			//首次访问首页
			to_page(1);
		});
			...
			$
					.each(
							...
							function(index, item) {
								...
								$("<tr></tr>").append(checkBoxId).append(
		...
		function build_page_info(result) {
			...
			maxPage = result.extend.pageInfo.total;
			currrentNum = result.extend.pageInfo.pageNum;
		}
		...
		function validate_add_Form() {
			...
			if (!regName.test(empName)) {
				//	alert("用户名可以是2-5位中文，3-16位英文和数字的组合")
				show_validate_msg("#empName_add_input", "error",
						"用户名可以是2-5位中文，3-16位英文和数字的组合");
				return false;
			} else {
				show_validate_msg("#empName_add_input", "success", "");
			}
			;
			var eamil = $("#empEmail_add_input").val();
		...
		$("#emp_update_btn")
				.click(
						function() {
							//验证邮箱是否合法
							var eamil = $("#empEmail_update_input").val();
							var regEmail = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
							if (!regEmail.test(eamil)) {
								//alert("邮箱格式不正确")
		...
		$("#emp_delete_all_btn").click(
				function() {
					...
					$.each($(".check_item:checked"), function() {
						empName += $(this).parents("tr").find("td:eq(2)")
								.text()
								+ ",";
						//组装id字符串
